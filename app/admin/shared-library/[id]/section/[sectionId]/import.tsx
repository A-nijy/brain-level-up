...
export default function AdminImportItemsScreen() {
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            const currentItems = await SharedLibraryService.getSharedItems(sid);
            let nextOrder = currentItems.length > 0 ? Math.max(...currentItems.map(i => i.display_order)) + 1 : 0;
            ...
            const itemsToInsert = parsedData.map((row: any) => {
                ...
                const item = {
                    ...
                    memo: mKey ? String(row[mKey]) : null,
...
const styles = StyleSheet.create({
    container: { flex: 1 },
    content: { padding: 32 },
    guideCard: { marginBottom: 32, padding: 0, overflow: 'hidden', borderRadius: 24, borderWidth: 1.5 },
    guideHeader: { flexDirection: 'row', alignItems: 'center', padding: 20, borderBottomWidth: 1 },
    guideTitle: { fontSize: 18, fontWeight: 'bold', marginLeft: 10 },
    guideBody: { padding: 20 },
    guideText: { fontSize: 15, marginBottom: 10, lineHeight: 22 },
    uploadButton: { height: 200, borderWidth: 2, borderStyle: 'dashed', borderRadius: 32, justifyContent: 'center', alignItems: 'center', overflow: 'hidden' },
    uploadText: { marginTop: 20, fontSize: 18, fontWeight: '700', paddingHorizontal: 40, textAlign: 'center' },
    changeText: { marginTop: 10, fontSize: 14, fontWeight: 'bold' },
    previewSection: { marginTop: 40 },
    previewTitle: { fontSize: 16, fontWeight: 'bold', marginBottom: 16, marginLeft: 8 },
    previewCard: { padding: 0, overflow: 'hidden', borderRadius: 24, borderWidth: 1.5 },
    previewItem: { padding: 20 },
    previewRowText: { fontSize: 14, fontFamily: Platform.OS === 'ios' ? 'Menlo' : 'monospace' },
    moreItems: { padding: 16, alignItems: 'center' },
    moreText: { fontSize: 14, fontStyle: 'italic' },
    footerGradient: { position: 'absolute', bottom: 0, left: 0, right: 0, padding: 32, paddingTop: 60 },
    importButton: { height: 64, borderRadius: 20, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', shadowColor: "#000", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.2, shadowRadius: 10, elevation: 5 },
    disabledButton: { opacity: 0.5 },
    importButtonText: { color: '#fff', fontSize: 18, fontWeight: '800' },
});
